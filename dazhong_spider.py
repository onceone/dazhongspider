...
def get_ip(num=10):
    headers = {
        'tid': 556843304900408,  # 订单号
        'num': num,  # 课一次提取n个
        'filter': 'on',  # g过滤24小时内提取过的，加on 过滤
        'protocol': 0,  # 默认普匿0+高匿2
        'delay': 25,  # 打开百度首页的时间
        'sortby': 'speed'  # time最新优先，speed最快优先
    }

    ip = requests.get('http://api3.xiguadaili.com/ip/?' +
                      urlencode(headers)).text
...
proxies = {'http': 'http://{}'.format(get_ip(1)[0]),
           'https': 'https://{}'.format(get_ip(1)[0])
           }
...
def get_html_origin_css_link(url):
    ...
    # 正则表达式匹配css链接,程序不稳定，有时候findall起作用，有时候不行，目前不知道为啥
    try:
        css_link = re.findall('<link rel="stylesheet" type="text/css" href="(//s3plus.*?\.css)">', html_origin, re.S)
        css_link = 'http:' + css_link[0]

    except:
        css_link = re.search('<link rel="stylesheet" type="text/css" href="(//s3plus.*?\.css)">', html_origin, re.S)
        ...
        return html_origin, html_css  # 返回网页源码和css样式链接


# ------------- down两种方法-------------------------
#   获取隐藏字体的class_name　前两个(or三个)字母，以下写生两个函数，方便调用

def get_front_two_alp(html_origin, class_name):
    """
    获取隐藏字体的class_name　前两个字母
    通过一级span的class_name,取出其子标签的class_name前两个字母
    """

    front_two_alp = re.search('<span class="' + class_name + '">.*?<svgmtsi class="(\w\w).*?"></span>', html_origin,
                              re.S)
    # print('打印地址的前两个字母')
    # print(front_two_alp.group(1))

    return front_two_alp.group(1)  # 返回地址的class_name的前两个字母


def get_front_three_alp(html_origin, class_name):
    """
    获取隐藏字体的class_name　前三个字母
    通过一级span的class_name,取出其子标签的class_name前三个字母
    """
    front_three_alp = re.search('<span class="' + class_name + '">.*?<svgmtsi class="(\w\w\w).*?"></span>', html_origin,
                                re.S)
    # print('打印地址的前三个字母')
    # print(front_three_alp.group(1))

    return front_three_alp.group(1)  # 返回地址的class_name的前三个字母


# -------------- up--------------------------------

def get_svg_link(html_css, alp):
    """获取的svg链接"""
    # print(alp)
    try:
    ...
    return background_image_link  # 加密字体对应此链接


def get_font_css(html_css, front_alp_addr):
    '''获取html_css里面有关地址的(class_name,x,y)'''

    font_css = re.findall('\.({0}\w+)'.format(front_alp_addr) + '{background:-(\d+).0px -(\d+).0px;}', html_css,
                          re.S)
    # print(font_css)

    return font_css  # 返回class_name对应的字典，（class_name,x,y)


# -----------down 3.为两种解密方法--------------------
# 3.1获得class_name 对应的文字 ,一共有两种方式，分别用两个函数表达，方便调用

def get_font_dict_by_offset_have_M(addr_svg_link, font_css):
    """获取坐标偏移的文字字典  (class_name:font)
    第一种加密方法(old)有M0"""
    ...
    if y_list_M0:
        ...
        return font_list  # 返回字典 (class_name:font)


# 3.2获得class_name 对应的文字  //第二种加密方法－new－无M0
def get_font_dict_by_offset_no_M(svg_link_num, food_kind_css):
    """获取坐标偏移的文字字典  (class_name:font)
    第一种加密方法(new)无M0"""
    # print(addr_css)
...
def get_food_kind_area(html_origin, html_css):
    ...
    font_list = get_font_dict_by_offset_have_M(food_kind__svg_link, food_kind_css)
...
def get_comment(html_origin, html_css):
    ...
    font_list = get_font_dict_by_offset_no_M(food_kind__svg_link, food_comment_css)
    # print('class_name对应的文字')
    # print(font_list)
...
def get_review_num(html_origin, html_css):
    font_two_alp = get_front_two_alp_num(html_origin, class_name='review-num')
    ...
    font_list = get_font_dict_by_offset_no_M(svg_link_num, comment_css)
...
def get_addrs(html_origin, html_css):
    ...
    # 3.2 获取css样式对应的地址字典文件  //第二种加密方法－old－有M0
    font_list = get_font_dict_by_offset_have_M(addr_svg_link, font_css)
...
def main():
    ...
    for i in range(1, page):
        ...
        if i == 1:
            url = 'https://www.dianping.com/search/keyword/2/10_{}'.format(para)
            # url = 'http://www.dianping.com/beijing/ch10/g311p{}'.format(num)
        else:
            url = 'https://www.dianping.com/search/keyword/2/10_{}/p{}'.format(para,i)
    ...
    for i in range(0,page-1):
        ...
        # print(shop_name_l[i])
        shop_name_num += shop_name_l[i]
        food_kind_num += food_kind_l[i]
        food_area_num += food_area_l[i]
        addr_num += addr_l[i]
        lat_num += lat_l[i]
        lng_num += lng_l[i]
    ...
    zipped = zip(shop_name_num, food_kind_num, food_area_num, addr_num, lat_num, lng_num)
    with open('table.csv', 'w') as f:
        writer = csv.writer(f)

        writer.writerow(['店名', '美食类别', '区', '详细地址', '纬度', '经度'])
        for i in zipped:
            writer.writerow(i)
